                            ...
                            <form action="{{ route('admin.products.update', $product->product_id) }}" method="POST" enctype="multipart/form-data">
                                ...
                                <div class="form-group">
                                    ...
                                    <select class="form-control @error('stock_unit') is-invalid @enderror" id="stock_unit" name="stock_unit">
                                        ...
                                        <option value="piece" {{ old('stock_unit', $product->stock_unit) == 'piece' ? 'selected' : '' }}>Piece</option>
                                        <option value="dozen" {{ old('stock_unit', $product->stock_unit) == 'dozen' ? 'selected' : '' }}>Dozen</option>
                                        <option value="box" {{ old('stock_unit', $product->stock_unit) == 'box' ? 'selected' : '' }}>Box</option>
                                    </select>
                                    @error('stock_unit')
                                    <div class="invalid-feedback">
                                        {{ $message }}
                                ...
                                <div class="form-group" id="stock_field" style="display: none;">
                                    ...
                                    <input type="number" class="form-control @error('stock') is-invalid @enderror" id="stock" name="stock" value="{{ old('stock', $product->stock) }}" readonly>
                                    @error('stock')
                                    <div class="invalid-feedback">
                                        {{ $message }}
                                    </div>
                                ...
                                <div class="form-group">
                                    ...
                                    <select class="form-control @error('sub_category_id') is-invalid @enderror" id="sub_category_id" name="sub_category_id">
                                        ...
                                        <option value="{{ $subcategory->sub_category_id }}" {{ $subcategory->sub_category_id == $product->sub_category_id ? 'selected' : '' }}>
                                ...
                                <div class="form-group">
                                    ...
                                    <select class="form-control @error('sub_sub_category_id') is-invalid @enderror" id="sub_sub_category_id" name="sub_sub_category_id">
                                        ...
                                        <option value="{{ $subsubcategory->sub_sub_category_id }}" {{ $subsubcategory->sub_sub_category_id == $product->sub_sub_category_id ? 'selected' : '' }}>
                                ...
                                <div class="form-group">
                                    ...
                                    <select class="form-control @error('skin_type') is-invalid @enderror" id="skin_type" name="skin_type">
                                        ...
                                        <option value="oily" {{ old('skin_type', $product->skin_type) == 'oily' ? 'selected' : '' }}>Oily</option>
                                        <option value="dry" {{ old('skin_type', $product->skin_type) == 'dry' ? 'selected' : '' }}>Dry</option>
                                        <option value="combination" {{ old('skin_type', $product->skin_type) == 'combination' ? 'selected' : '' }}>Combination</option>
                                        <option value="sensitive" {{ old('skin_type', $product->skin_type) == 'sensitive' ? 'selected' : '' }}>Sensitive</option>
                                        <option value="normal" {{ old('skin_type', $product->skin_type) == 'normal' ? 'selected' : '' }}>Normal</option>
                                    </select>
                                    @error('skin_type')
                                    <div class="invalid-feedback">
                                        {{ $message }}
                                ...
                                <div class="form-group">
                                    ...
                                    <input type="text" class="form-control @error('dimension') is-invalid @enderror" id="dimension" name="dimension" placeholder="e.g., 2x2x2x3" value="{{ old('dimension', $product->dimension) }}">
                                    @error('dimension')
                                    <div class="invalid-feedback">
                                        {{ $message }}
                                    </div>
                                ...
                                <div class="form-group">
                                    ...
                                    <select class="form-control @error('volume_unit') is-invalid @enderror" id="volume_unit" name="volume_unit">
                                        ...
                                        <option value="ml" {{ old('volume_unit', $product->volume_unit) == 'ml' ? 'selected' : '' }}>ml</option>
                                        <option value="grams" {{ old('volume_unit', $product->volume_unit) == 'grams' ? 'selected' : '' }}>grams</option>
                                        <option value="liters" {{ old('volume_unit', $product->volume_unit) == 'liters' ? 'selected' : '' }}>liters</option>
                                        <option value="kg" {{ old('volume_unit', $product->volume_unit) == 'kg' ? 'selected' : '' }}>kg</option>
                                        <option value="oz" {{ old('volume_unit', $product->volume_unit) == 'oz' ? 'selected' : '' }}>oz</option>
                                        <option value="fl oz" {{ old('volume_unit', $product->volume_unit) == 'fl oz' ? 'selected' : '' }}>fl oz</option>
                                    </select>
                                    @error('volume_unit')
                                    <div class="invalid-feedback">
                                        {{ $message }}
                                ...
                                <div class="form-group" id="volume_field" style="display: none;">
                                    ...
                                    <input type="text" class="form-control @error('volume') is-invalid @enderror" id="volume" name="volume" placeholder="e.g., 500" value="{{ old('volume', $product->volume) }}">
                                    @error('volume')
                                    <div class="invalid-feedback">
                                        {{ $message }}
                                    </div>
                                ...
                                <div class="form-group">
                                    ...
                                    @enderror
                                    @if($product->image)
                                    <img src="{{ asset('storage/' . $product->image) }}" alt="{{ $product->product_name }}" style="width: 100px; height: 100px; margin-top: 10px;">
                                    @endif
                                </div>
                                <div class="form-group">
                                    <label for="date_manufactured">Date Manufactured</label>
                                    <input type="text" class="form-control @error('date_manufactured') is-invalid @enderror" id="date_manufactured" name="date_manufactured" value="{{ old('date_manufactured', $product->date_manufactured) }}" placeholder="YYYY-MM-DD">
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        // Show the stock field if a stock unit is selected
        ...
        // Show the volume field if a volume unit is selected
        ...
        // Initialize the stock and volume fields based on the product data
        document.addEventListener('DOMContentLoaded', function() {
            var stockUnit = document.getElementById('stock_unit');
            var volumeUnit = document.getElementById('volume_unit');
            var stockField = document.getElementById('stock_field');
            var volumeField = document.getElementById('volume_field');

            // Set the stock unit and volume unit based on the product data
            if (stockUnit) {
                stockUnit.value = "{{ $product->stock_unit }}";
                stockUnit.dispatchEvent(new Event('change'));
            }

            if (volumeUnit) {
                volumeUnit.value = "{{ $product->volume_unit }}";
                volumeUnit.dispatchEvent(new Event('change'));
            }

            // Set the stock and volume values based on the product data
            if (stockField) {
                stockField.value = "{{ $product->stock }}";
            }

            if (volumeField) {
                volumeField.value = "{{ $product->volume }}";
            }
        });
    });
</script>